...
class DuckDbHook(BaseHook):  # pylint: disable=abstract-method
    ...
    def run_cli(
        ...
        *,
        ...
        """
        Execute an inline SQL statement via DuckDB CLI.
        ...
        cmd = self._build_command(params, output_format=output_format, sql=sql)
    ...
    def _run(self, cmd: list[str], *, timeout: int) -> str:
        ...
        try:
            ...
            raise AirflowException(
                f"DuckDB binary not found: '{cmd[0]}'. "
                "Install DuckDB CLI or set 'duckdb_binary' in the connection extra."
            ) from exc
