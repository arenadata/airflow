...
It performs the same operations as ozone_usage_example.py but uses encrypted connections.
...
if _HAS_S3:
    ...
    with DAG(
        "ozone_usage_ssl_example",
        default_args=default_args,
        description="Example DAG demonstrating Ozone operations with SSL/TLS (requires apache-airflow-providers-amazon)",
        schedule=None,
        start_date=datetime(2024, 1, 1),
        catchup=False,
        tags=["ozone", "example"],
        ...
        BashOperator(
            task_id="requires_amazon_provider",
            bash_command="echo 'Install apache-airflow-providers-amazon to run the full example.'; exit 0",
        )
