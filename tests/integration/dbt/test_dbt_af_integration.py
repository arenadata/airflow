...
class TestManifestParsing:
    ...
    def test_manifest_exists(self):
        """manifest.json is generated by 'dbt compile' (conftest.py runs it automatically)"""
        assert Path(MANIFEST_PATH).exists(), "manifest.json must exist in target/"

    def test_manifest_has_required_sections(self):
        """nodes = models/tests/snapshots/seeds, sources = external tables, metadata = dbt version info"""
    ...
    def test_manifest_node_counts(self):
        """Demo project: 6 SQL models, 4 singular tests, 1 SCD snapshot"""
        with open(MANIFEST_PATH) as f:
            manifest = json.load(f)
        by_type = {}
    ...
    def test_manifest_sources_count(self):
        """3 external source tables: orders, customers, customers_scd (defined in sources.yml)"""
        with open(MANIFEST_PATH) as f:
            manifest = json.load(f)
...
class TestDagGeneration:
    ...
    def test_dags_not_empty(self, generated_dags):
        """compile_dbt_af_dags must produce at least one DAG from the manifest"""
        assert len(generated_dags) > 0

    def test_all_dags_have_dbt_tag(self, generated_dags):
        """dbt-af tags every generated DAG with 'dbt' for filtering in Airflow UI"""
    ...
    def test_manual_run_dag_exists(self, generated_dags):
        """When include_single_model_manual_dag=True, dbt-af creates a special DAG
...
class TestDomainSeparation:
    ...
    def test_multiple_domain_dags_created(self, generated_dags):
        """Each domain (models/<domain>/) + schedule combination produces a separate DAG
...
class TestBackfillDags:
    ...
    def test_backfill_dags_exist(self, generated_dags):
        """dbt-af creates a backfill DAG for each domain to re-run historical data"""
